...
public class MainManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < LineCount; ++i)
        {
            ...
            {
                Vector3 position = new(-1.5f + (step * x),
                                       2.5f + (i * 0.3f),
                                       0);
    ...
    public void GameOver()
    {
        ...
        if (m_Points > DataPersistenceManager.Instance.CurrentPlayerBestScore)
        {
            DataPersistenceManager.Instance.CurrentPlayerBestScore = m_Points;
        }

        if (DataPersistenceManager.Instance.CurrentPlayerBestScore > DataPersistenceManager.Instance.BestScore)
        {
